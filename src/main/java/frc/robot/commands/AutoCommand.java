...
public class AutoCommand extends SequentialCommandGroup {
  ...
  public AutoCommand(DriveTrain dT, Belt b, EyesValues eV, CameraInfo cI, Shooter s ) {
    driveTrain = dT;
    shooter = s;
    belt = b;
    eyesValues = eV;
   ...
   addCommands(
     ...
     new AutoDrive(driveTrain, 0, 1, 0).withTimeout(1.45),
     new AutoParallel(driveTrain, belt, eyesValues).withTimeout(4),
     new AutoDrive(driveTrain, 0, 0, 0).withTimeout(0.05),
     new AutoDrive(driveTrain, 0, -1, 0).withTimeout(0.75),
     new AutoDrive(driveTrain, -1, 0, 0).withTimeout(0.75), // positive lateral motion goes right.
     new AutoDrive(driveTrain,0, -1, 0).withTimeout(0.15),
